  ...
  function ProductLayout({ children }, ref) {
    ...
    useEffect(() => {
      if (isOpen && results.length > 0) {
        const lastResult: ResultType = results[
          results.length - 1
        ] as ResultType;
        if (location.pathname !== routes.CUSTOMER.HOME) {
          navigate(
            `${routes.CUSTOMER.HOME}?search=${lastResult.transcript.trim()}`
          );
        } else {
          const searchParams = new URLSearchParams(location.search);
          const param = Object.fromEntries(searchParams.entries());
          const newParams = { ...param, title: lastResult.transcript.trim() };
          const newSearchParams = new URLSearchParams();
          Object.entries(newParams).forEach(([key, value]) => {
            if (value !== undefined && value !== null && value !== "") {
              newSearchParams.set(key, String(value));
          ...
          <DialogContent className="bg-gray-100">
            <div className="h-28 w-full flex items-center justify-center overflow-hidden ">
              <div
                className={`relative w-20 h-20 rounded-full flex items-center justify-center ${
                  isRecording ? "bg-red-600 shadow-lg" : "bg-white shadow-md"
                } transition-all duration-500`}
                onClick={toggleRecording}
              >
                <div
                  className={`absolute w-20 h-20 rounded-full opacity-20 ${
                    isRecording ? "bg-gray-500 animate-ping" : "bg-transparent"
                  }`}
                ></div>
                <Mic
                  className={`fas w-8 h-8 fa-microphone text-2xl transition-all ${
            ...
            <div className="text-center">
              {isRecording ? (
                ...
                <h1>Nhấn vào biểu tượng micro để thử lại.</h1>
              )}
            </div>
          </DialogContent>
        </Dialog>
        <div className="h-screen md:w-full w-full grid grid-rows-[96px_1fr] overflow-y-auto overflow-x-hidden md:overflow-hidden md:pb-0 pb-10">
          <div className="md:px-40 px-0 h-full w-full flex flex-row items-center bg-[#fff] gap-0 md:gap-16 border-b">
            ...
            <div className="flex flex-row gap-1 md:gap-4 flex-1">
              ...
              <div
                className="rounded-full w-10 h-10 p-1 bg-[#ffe1c3] border-2 flex items-center justify-center"
                onClick={() => {
                  setIsOpen(true);
                  startSpeechToText();
                }}
              >
                <Mic className="w-2/3 h-2/3 text-[#A93F15] hover:text-[#A93F15]" />
              </div>
            </div>
            {!auth && (
