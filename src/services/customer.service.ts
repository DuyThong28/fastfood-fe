...
class CustomerService {
  async getAllCusomter(
    { page, take }: Page,
    disable: boolean | null,
  ): Promise<ResFetchAllCustomers> {
    if (disable === true || disable === false) {
      return api.get(
        `/users/get-all?page=${page}&take=${take}&disable=${disable}`,
      );
    } else {
  ...
  async updateAccount(
    ...
    if (trimmedData.birthday)
      formData.append("birthday", trimmedData.birthday?.toString());
    ...
    if (trimmedData.phone)
      formData.append("phone", trimmedData.phone?.toString());
    if (imageFile) {
